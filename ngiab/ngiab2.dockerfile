#-----------------------------------------------------------------------
# Note: The following error message occurs intermittently:
...
# Python venv was added to path above with:
#     ENV PATH="/ngen/.venv/bin:$PATH"
# so it is presumably still "activated"
#===================================================================
FROM ngen_clone AS tf36_build
...
#---------------------------------------------------------------
# Note: epel-release, python3, python3-pip, python3-devel
#       were installed in build_base above and they are in
#       ngen_clone because:
# build_base -> boost_build -> troute_prebuild -> troute_build
# build_base -> boost_build -> troute_prebuild -> ngen_clone
#---------------------------------------------------------------
# Next 2 lines are in "stand-alone" gdal6.dockerfile.
#-----------------------------------------------------------  
## RUN dnf install -y epel-release && dnf clean all
## RUN dnf install -y python3 python3-pip python3-devel
#------------------------------------------------------
# SDP.  Next line installs GDAL 3.4.3. for Python 3.9
#------------------------------------------------------
...
# Install requirements like this so the troute clone can
# can run in parallel with ngen download and build
...
#---------------------------------------------------------
# SDP.  Copy everything needed by GDAL from tf36_build
#       Likely need something similar for matplotlib.
#---------------------------------------------------------
COPY --from=tf36_build /usr/lib/libgdal* /usr/lib/
COPY --from=tf36_build /usr/lib64/libgdal* /usr/lib64/
COPY --from=tf36_build /usr/lib/python3.9/site-packages/osgeo* /usr/lib/python3.9/site-packages
COPY --from=tf36_build /usr/share/gdal /usr/share/gdal
COPY --from=tf36_build /usr/bin/gdal* /usr/bin
COPY --from=tf36_build /usr/bin/ogr* /usr/bin
COPY --from=tf36_build /usr/share/gdal /usr/share/gdal
#--------------------------------------------------------------
...
#--------------------------------------------------------------
## COPY --from=tf36_build /usr/include/gdal /usr/include/gdal

#-----------------------------------------------------------
# SDP.  Install topoflow36 package here.
#-------------------------------------------------------------
# It is a submodule of NGEN_REPO, and was installed above in 
#   the "ngen_clone" build stage via "git submodule update".
#   Recall tf36_build stage inherits from ngen_clone.
# Emulate COPY & RUN commands above to install LSTM.
# Install after venv added to PATH.
# If needed, can activate venv like this:
#    RUN . /ngen/.venv/bin/activate
#-------------------------------------------------------------
COPY --from=tf36_build /ngen/ngen/extern/topoflow36 /ngen/ngen/extern/topoflow36
